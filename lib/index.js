...
var DataStore = class {
  ...
  listWorkspaceSubdirs(userId) {
    ...
    return entries.filter((name2) => {
      try {
        return fs.statSync(path.join(root, name2)).isDirectory();
      } catch {
        return false;
      }
    }).sort((a, b) => a.localeCompare(b)).map((name2) => ({ name: name2, path: path.join(root, name2) }));
  }
  /** 在专属目录下新建一个子目录（= 新建工作区），返回其绝对路径。 */
  createWorkspaceSubdir(userId, name2) {
    const safe = String(name2 || "").trim();
  ...
  deleteWorkspaceSubdir(userId, name2) {
    const safe = String(name2 || "").trim();
    if (!safe || safe.includes("/") || safe.includes("\\") || safe === "." || safe === "..") {
