...
var DataStore = class {
  ...
  saveGrants(userId, workspaceDirs) {
    writeJsonAtomic(this.grantsPath(userId), { workspaceDirs, updatedAt: (/* @__PURE__ */ new Date()).toISOString() });
  }
};
...
async function handleAdmin(store, lifecycle, req, res, pathname) {
  ...
  if (key === "POST /api/mu/admin/users") {
    ...
    const dirs = Array.isArray(body.workspaceDirs) ? body.workspaceDirs.filter((d) => typeof d === "string") : [];
