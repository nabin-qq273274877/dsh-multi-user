...
var DataStore = class {
  ...
  addWorkspacePath(userId, p) {
    ...
    writeJsonAtomic(this.grantsPath(userId), { ...record, workspacePaths: list, updatedAt: (/* @__PURE__ */ new Date()).toISOString() });
    return list;
  }
  /** 从用户清单移除一条路径（按规范化比较），返回更新后的清单。 */
  removeWorkspacePath(userId, p) {
    const record = readJson(this.grantsPath(userId)) ?? {};
    const list = Array.isArray(record.workspacePaths) ? record.workspacePaths.slice() : [];
    const norm = normalizeForCompare(p);
    const next = list.filter((x) => normalizeForCompare(x) !== norm);
    writeJsonAtomic(this.grantsPath(userId), { ...record, workspacePaths: next, updatedAt: (/* @__PURE__ */ new Date()).toISOString() });
    return next;
  }
};
